#!/usr/bin/env python3
"""Owner follow-up: Selawik bar buttons and restrained ELAN pointer/scroll."""
...
def main():
    ...
    rels=['quickshell/apx/BarButton.qml','hypr/hyprland.lua']
    ...
    for name in ['hub','faculdade','hytale','minecraft','steam']:
        ...
        p=env/'home/apx/.config'/rels[0]
        assert p.read_bytes()==(SEED/rels[0]).read_bytes()
        planned.append((p,(SOURCE/rels[0]).read_bytes()))
    ...
    backup=Path('/var/lib/apx/backups')/(datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-touchpad-refine');backup.mkdir(mode=0o700)
    manifest=[]
