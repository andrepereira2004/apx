...
def main():
    ...
    rels=['quickshell/apx/BarButton.qml','hypr/hyprland.lua','quickshell/apx/shell.qml','apx/bashrc','local/bin/apx-sysinfo']
    marker='-- Owner-requested touchpad trial:'
    old=marker+(SEED/rels[1]).read_text().split(marker,1)[1]
    new=marker+(SOURCE/rels[1]).read_text().split(marker,1)[1]
    planned=[]
    for name in ['hub','faculdade','hytale','minecraft','steam']:
        ...
        for rel in (rels[0], rels[2], rels[3], rels[4]):
            if name=="hub" and rel=="local/bin/apx-sysinfo": continue
            p=env/'home/apx'/('.'+rel if rel.startswith('local/') else '.config/'+rel)
            assert (p.is_file() and p.read_bytes()==(SEED/rel).read_bytes()) or (name=='hub' and rel in rels[3:] and not p.exists())
            planned.append((p,(SOURCE/rel).read_bytes()))
        p=env/'home/apx/.config'/rels[1];text=p.read_text();assert text.count(old)==1
        planned.append((p,text.replace(old,new).encode()))
    p=BASE/'hub/home/apx/.config/kitty/kitty.conf'
    t=p.read_text()
    assert not any(line.startswith('shell ') for line in t.splitlines())
    planned.append((p,(t+'\nshell /usr/bin/bash --rcfile /home/apx/.config/apx/bashrc -i\n').encode()))
    ...
    backup=Path('/var/lib/apx/backups')/(datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-menu-identity');backup.mkdir(mode=0o700)
    manifest=[]
    for i,(p,data) in enumerate(planned):
        assert not p.is_symlink()
        exists=p.exists()
        st=p.stat() if exists else (BASE/'hub/home/apx').stat()
        saved=backup/str(i)
        if exists: shutil.copy2(p,saved)
        mode=(st.st_mode&0o777) if exists else (0o755 if p.name=='apx-sysinfo' else 0o644)
        manifest.append(dict(target=str(p),backup=str(saved) if exists else None,uid=st.st_uid,gid=st.st_gid,mode=mode,before=hashlib.sha256(p.read_bytes()).hexdigest() if exists else None,after=hashlib.sha256(data).hexdigest()))
