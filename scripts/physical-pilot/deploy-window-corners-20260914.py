...
def main():
    ...
    rels=['quickshell/apx/shell.qml','quickshell/apx/WindowCornerControls.qml','local/bin/apx-window-corner-watch','rofi/config.rasi']
    planned=[]
    ...
    backup=Path('/var/lib/apx/backups')/(datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-window-corners');backup.mkdir(mode=0o700)
    manifest=[]
