...
echo APX Windows: A iniciar
set "APX_EXPECTED=X:\Windows\System32\apx-expected.ini"
set "APX_LOG=X:\apx-native-windows-install-v3.log"
set "APX_DP_SCRIPT=X:\apx-diskpart-v3.txt"
set "APX_DP_OUTPUT=X:\apx-diskpart-v3.out"
set "APX_MOUNT_LIST=X:\apx-mountvol-v3.out"
set "APX_IMAGE_INFO=X:\apx-image-index-v3.out"
set "APX_LAST_COMMAND=initialization"
set "APX_LAST_EXIT=0"
set "APX_DIAGNOSTIC=none"

>"%APX_LOG%" echo APX native Windows installer v3 started: windows-2.
call :load_expected || call :fatal APX-CONTRACT-01 expected-contract

set "APX_STEP=disk-identity"
echo APX Windows: A verificar o SSD
call :find_disk || call :fatal APX-DISK-01 disk-identity

set "APX_STEP=partition-identities"
echo APX Windows: A verificar as partições
...
call :validate_contract "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-contract-v3.ini" || call :fatal APX-CONTRACT-02 setup-contract
call :find_partition EFI "c12a7328-f81f-11d2-ba4b-00a0c93ec93b" "APXE29758BB" "512 MB" || call :fatal APX-PART-01 apx-efi
call :choose_letter EFI S || call :fatal APX-LETTER-03 apx-efi
call :mount_partition !APX_PART_EFI! !APX_LETTER_EFI! EFI "APXE29758BB" || call :fatal APX-MOUNT-03 apx-efi
set "APX_ESP=!APX_LETTER_EFI!:"
call :validate_contract "%APX_ESP%\EFI\APX\native-windows\install-contract-v3.ini" || call :fatal APX-CONTRACT-04 efi-contract
...
call :mount_partition !APX_PART_WINDOWS! !APX_LETTER_WINDOWS! WINDOWS "APXW29758BB" || call :fatal APX-MOUNT-01 windows-target

set "APX_TARGET=!APX_LETTER_WINDOWS!:"
call :validate_contract "%APX_TARGET%\APX\install-contract-v3.ini" || call :fatal APX-CONTRACT-03 target-contract
...
call :validate_contract "%APX_TARGET%\APX\install-contract-v3.ini" || call :fatal APX-FORMAT-00 target-contract-revalidation
>"%APX_DP_SCRIPT%" echo select disk !APX_DISK!
>>"%APX_DP_SCRIPT%" echo select partition !APX_PART_WINDOWS!
>>"%APX_DP_SCRIPT%" echo format fs=ntfs quick label=APXW29758BB
...
copy /B /Y "%APX_EXPECTED%" "%APX_TARGET%\APX\install-contract-v3.ini" >>"%APX_LOG%" 2>&1
if errorlevel 1 call :fatal APX-FORMAT-02 restore-target-contract
call :validate_contract "%APX_TARGET%\APX\install-contract-v3.ini" || call :fatal APX-FORMAT-03 formatted-target-contract
vol %APX_TARGET% >"%APX_DP_OUTPUT%" 2>&1
if errorlevel 1 call :fatal APX-FORMAT-04 formatted-target-volume
X:\Windows\System32\find.exe /I "APXW29758BB" "%APX_DP_OUTPUT%" >nul || call :fatal APX-FORMAT-04 formatted-target-label

set "APX_STEP=apply-windows-11-pro"
echo APX Windows: A instalar o Windows
set "APX_LAST_COMMAND=dism-apply-image"
X:\Windows\System32\dism.exe /Apply-Image /ImageFile:%APX_MEDIA%\sources\install.swm /SWMFile:%APX_MEDIA%\sources\install*.swm /Index:6 /ApplyDir:%APX_TARGET%\ /LogPath:%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\dism-apply-v3.log /LogLevel:4 >>"%APX_LOG%" 2>&1
set "APX_LAST_EXIT=!ERRORLEVEL!"
>>"%APX_LOG%" echo COMMAND !APX_LAST_COMMAND! exit=!APX_LAST_EXIT! log=%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\dism-apply-v3.log
...
echo APX Windows: A preparar rede e integração APX
...
set "APX_STEP=display-drivers"
echo APX Windows: a preparar os controladores dos monitores.
call :find_partition SOURCE "ebd0a0a2-b9e5-4433-87c0-68b6b72699c7" "-" "120 GB" || call :fatal APX-DRIVER-02 source-partition
call :choose_letter SOURCE T || call :fatal APX-DRIVER-03 source-letter
call :mount_basic_partition !APX_PART_SOURCE! !APX_LETTER_SOURCE! SOURCE "APXWINTARGET" || call :fatal APX-DRIVER-04 source-volume
set "APX_SOURCE=!APX_LETTER_SOURCE!:"
call :validate_contract "%APX_SOURCE%\APX-29758bbf-bc17-4b73-8db9-15cf1f0d9725.ini" || call :fatal APX-DRIVER-05 source-contract
X:\Windows\System32\dism.exe /Image:%APX_TARGET%\ /Add-Driver /Driver:"%APX_SOURCE%\Windows\System32\DriverStore\FileRepository\nvlt.inf_amd64_18cae871934f9f98" /Recurse >>"%APX_LOG%" 2>&1
if errorlevel 1 call :fatal APX-DRIVER-06 display-driver-1
X:\Windows\System32\dism.exe /Image:%APX_TARGET%\ /Add-Driver /Driver:"%APX_SOURCE%\Windows\System32\DriverStore\FileRepository\u0386187.inf_amd64_e45f6929dd019ce6" /Recurse >>"%APX_LOG%" 2>&1
if errorlevel 1 call :fatal APX-DRIVER-06 display-driver-2
X:\Windows\System32\dism.exe /Image:%APX_TARGET%\ /Add-Driver /Driver:"%APX_SOURCE%\Windows\System32\DriverStore\FileRepository\u0386195.inf_amd64_0e79be229d9800f5" /Recurse >>"%APX_LOG%" 2>&1
if errorlevel 1 call :fatal APX-DRIVER-06 display-driver-3

set "APX_STEP=windows-boot-manager"
echo APX Windows: A preparar o arranque
...
X:\Windows\System32\bcdedit.exe /store %APX_ESP%\EFI\Microsoft\Boot\BCD /enum {default} /v >"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\bcd-loader-v3.txt" 2>&1
if errorlevel 1 call :fatal APX-BCD-04 bcd-loader-enumeration
call :file_has_key_exact "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\bcd-loader-v3.txt" "device" "partition=%APX_TARGET%" || call :fatal APX-BCD-05 loader-device
call :file_has_key_exact "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\bcd-loader-v3.txt" "osdevice" "partition=%APX_TARGET%" || call :fatal APX-BCD-06 loader-osdevice
call :file_has_key_exact "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\bcd-loader-v3.txt" "path" "\Windows\system32\winload.efi" || call :fatal APX-BCD-07 loader-path
rem No global BCD or firmware writes; Linux registers the exact new EFI partition.
if not exist "%APX_MEDIA%\sources\install.swm" call :fatal APX-BCD-18 setup-preservation

set "APX_STEP=success-record"
echo APX Windows: Instalação concluída
>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo profile=apx-native-windows-install-status-v3
>>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo generation=!APX_generation!
>>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo status=boot-prepared
>>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo image_index=6
>>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo windows_partition_guid=!APX_windows_partition_guid!
>>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo esp_partition_guid=!APX_efi_partition_guid!
>>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo target_letter=!APX_LETTER_WINDOWS!:
>>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo esp_letter=!APX_LETTER_EFI!:
copy /B /Y "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" "%APX_ESP%\EFI\APX\native-windows\install-status-v3.ini" >>"%APX_LOG%" 2>&1 || call :fatal APX-STATUS-01 efi-status-copy
X:\Windows\System32\fc.exe /B "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" "%APX_ESP%\EFI\APX\native-windows\install-status-v3.ini" >nul
if errorlevel 1 call :fatal APX-STATUS-02 efi-status-validation
copy /B /Y "%APX_LOG%" "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-log-v3.txt" >nul 2>&1
...
if not "!APX_profile!"=="apx-native-windows-install-contract-v3" exit /b 1
if not "!APX_name!"=="windows-2" exit /b 1
if not "!APX_generation!"=="29758bbf-bc17-4b73-8db9-15cf1f0d9725" exit /b 1
if not "!APX_plan_sha256!"=="1416c7eb48883778df021373ed2a4e8c4e4fc6e7b7f1067aa0ad2ea5aa5e679e" exit /b 1
if not "!APX_size_gib!"=="80" exit /b 1
...
if not "!APX_efi_partition_guid!"=="47E164FC-4736-5C6A-9302-C037D2398CFE" exit /b 1
if not "!APX_efi_start_sector!"=="812486656" exit /b 1
if not "!APX_efi_sector_count!"=="1048576" exit /b 1
...
if not "!APX_linux_sector_count!"=="558729216" exit /b 1
if not "!APX_windows_partition_guid!"=="07BB2472-589C-5382-84F4-DF4A81C74CEC" exit /b 1
if not "!APX_windows_start_sector!"=="813568000" exit /b 1
if not "!APX_windows_sector_count!"=="167772160" exit /b 1
...
if not "!APX_windows_label!"=="APXW29758BB" exit /b 1
if not "!APX_efi_label!"=="APXE29758BB" exit /b 1
if not "!APX_source_partition_guid!"=="099C31D8-313A-4ABA-B0E0-2B59502C9674" exit /b 1
if not "!APX_display_driver_1!"=="nvlt.inf_amd64_18cae871934f9f98" exit /b 1
if not "!APX_display_driver_2!"=="u0386187.inf_amd64_e45f6929dd019ce6" exit /b 1
if not "!APX_display_driver_3!"=="u0386195.inf_amd64_0e79be229d9800f5" exit /b 1
set "APX_WINDOWS_SIZE_TEXT=80 GB"
set "APX_LINUX_SIZE_TEXT=266 GB"
exit /b 0
...
if defined APX_MEDIA if exist "%APX_MEDIA%\APX\" (
    >"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo profile=apx-native-windows-install-status-v3
    >>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo generation=!APX_generation!
    >>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo status=failed
    >>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo error=!APX_ERROR!
    >>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo step=!APX_STEP!
    >>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo detail=!APX_DETAIL!
    >>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo command=!APX_LAST_COMMAND!
    >>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo exit_code=!APX_LAST_EXIT!
    >>"%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" echo diagnostic=!APX_DIAGNOSTIC!
    copy /B /Y "%APX_LOG%" "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-log-v3.txt" >nul 2>&1
)
if defined APX_ESP if exist "%APX_ESP%\EFI\APX\native-windows\" (
    if defined APX_MEDIA if exist "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" copy /B /Y "%APX_MEDIA%\APX\Native\windows-2\29758bbf-bc17-4b73-8db9-15cf1f0d9725\install-status-v3.ini" "%APX_ESP%\EFI\APX\native-windows\install-status-v3.ini" >nul 2>&1
    copy /B /Y "%APX_LOG%" "%APX_ESP%\EFI\APX\native-windows\install-log-v3.txt" >nul 2>&1
)
echo.
