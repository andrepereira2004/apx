...
def main():
    ...
    rels=['quickshell/apx/BarButton.qml','hypr/hyprland.lua','quickshell/apx/shell.qml','local/libexec/apx-system-power-client-v1.py']
    ...
    for name in ['hub','faculdade','hytale','minecraft','steam']:
        ...
        for rel in (rels[0], rels[2], rels[3]):
            p=env/'home/apx'/('.'+rel if rel.startswith('local/') else '.config/'+rel)
            assert p.read_bytes()==(SEED/rel).read_bytes()
    ...
    for name in ['apx-environment-hardware-v1.py','apx-system-power-runner-v1.py']:
        planned.append((Path('/usr/lib/apx')/name,(REPO/'scripts/physical-pilot'/name).read_bytes()))
    backup=Path('/var/lib/apx/backups')/(datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-environment-shutdown');backup.mkdir(mode=0o700)
    manifest=[]
    for i,(p,data) in enumerate(planned):
        assert p.is_file() and not p.is_symlink();st=p.stat();saved=backup/str(i);shutil.copy2(p,saved)
        manifest.append(dict(target=str(p),backup=str(saved),uid=st.st_uid,gid=st.st_gid,mode=st.st_mode&0o777,before=hashlib.sha256(p.read_bytes()).hexdigest(),after=hashlib.sha256(data).hexdigest()))
