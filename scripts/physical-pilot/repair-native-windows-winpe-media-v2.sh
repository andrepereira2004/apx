...
readonly mount_dir=/run/apx-windows-winpe-repair-v2
readonly staging_dir=/var/lib/apx/package-artifacts/system-images-v1/apx-winpe-repair-v2
readonly backup=/var/lib/apx/backups/20260825-native-windows-winpe-media-v2
readonly boot_wim_hash=e0e43465687fcb3daa3f6a521af183efd4e5e2de5e211934e085061f81d7323a
...
fail() { /usr/bin/printf 'APX WinPE media v2 repair refused: %s\n' "$1" >&2; exit 2; }
cleanup() {
    status=$?
    trap - EXIT INT TERM
...
/usr/bin/wimlib-imagex dir "$mount_dir/sources/boot.wim" 2 \
    --path=Windows/System32/mountvol.exe >/dev/null \
    || fail 'WinPE mountvol executable is absent'
...
trap - EXIT INT TERM
/usr/bin/printf 'APX WinPE EFI media mount repair v2 installed.\n'
